...
function percentual_acertoHOG = classificarSinais(percentual_teste)
  if nargin < 1
      percentual_teste = 0.3;%Percentual recebe um valor padr�o caso n�o seja fornecido
  end
  
  %----- Declara��o e inicializa��o pr�via de algumas das vari�veis ----- 
  contadorSinais=1;
  quant_sinais = zeros(1,7);
  diretorio_raiz = 'C:\Users\julia\Documents\UFOP\8_Semestre\PDI\TP\TrabalhoFinal\BancodeImagens\';%Diret�rio do banco de imagens
  ext = '.png';
  etiquetas = zeros(397,1);
  percentual_acertoHOG = zeros(7,1);
  
  %---------------- VARI�VEIS REFERENTES AO DESCRITOR HOG ---------------- 
  ...
  %PREENCHE O VETOR DE ETIQUETAS DE ACORDO COM O CONHECIMENTO PR�VIO
  for i=1:397
      if(i<=57)
          etiquetas(i) = 1;%R�tulo num�rico para o sinal da letra 'A'
      end
      if(i>57 && i<=113)
          etiquetas(i) = 2;%R�tulo num�rico para o sinal da letra 'D'
      end
      if(i>113 && i<=170)
          etiquetas(i) = 3;%R�tulo num�rico para o sinal da letra 'I'
      end
      if(i>170 && i<=226)
          etiquetas(i) = 4;%R�tulo num�rico para o sinal da letra 'L'
      end
      if(i>226 && i<=282)
          etiquetas(i) = 5;%R�tulo num�rico para o sinal da letra 'V'
      end
      if(i>282 && i<=339)
          etiquetas(i) = 6;%R�tulo num�rico para o sinal da letra 'W'
      end
      if(i>339 && i<=397)
          etiquetas(i) = 7;%R�tulo num�rico para o sinal da letra 'Y'
      end
  end
  ...
  %---------- TREINAMENTO E CLASSIFICA��O PARA O DESCRITOR HOG -----------
